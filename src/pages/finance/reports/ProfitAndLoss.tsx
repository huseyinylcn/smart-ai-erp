...
const ProfitAndLoss = () => {
  ...
  const mockValues = (base: number, volatility: number = 0.2) => {
    return periods.map(() => base * (1 + (Math.random() * volatility * 2 - volatility)));
  };

  const reportData: PnLRow[] = useMemo(() => [
    {
      id: '1',
      label: 'GƏLİRLƏR',
      isHeader: true,
      ...
      values: [],
      children: [
        { id: '1-1', label: 'Satışdan gəlirlər', code: '601', level: 1, values: periods.map(() => 80000) },
      ]
    },
    {
      id: 'gross_profit',
      label: 'ÜMUMİ MƏNFƏƏT (GROSS PROFIT)',
      isTotal: true,
      level: 0,
      values: periods.map(() => 80000)
    },
    {
      id: '2',
      label: 'ƏMƏLİYYAT XƏRCLƏRİ',
      isHeader: true,
      level: 0,
      values: [],
      children: [
        { id: '2-1', label: 'Satılan malın maya dəyəri (COGS)', code: '701', level: 1, values: periods.map(() => 50000) },
        { id: '2-2', label: 'İnzibati xərclər (Əmək Haqqı)', code: '711', level: 1, values: periods.map(() => 10000) },
      ]
    },
    {
      id: 'ebitda',
      label: 'EBITDA (Əməliyyat Mənfəəti)',
      isTotal: true,
      level: 0,
      values: periods.map(() => 20000)
    },
    {
      id: 'net_income',
      label: 'XALİS MƏNFƏƏT (NET INCOME)',
      isTotal: true,
      level: 0,
      values: periods.map(() => 20000)
    }
    ...
    return (
      <React.Fragment key={row.id}>
        <tr className={`
          ...
          ${row.isTotal ? 'bg-indigo-50/20 dark:bg-indigo-900/10' : 'hover:bg-slate-50/50'}
        `}>
          ...
          {periods.map((p, idx) => (
            <td key={p.key} className={`
              ...
              ${row.isTotal ? 'font-black text-indigo-600 dark:text-indigo-400 text-sm' : 'font-bold text-slate-700 dark:text-slate-200 text-xs'}
              ${idx % 2 === 0 ? 'bg-slate-50/20' : ''}
              ${!row.isHeader && !row.isTotal ? 'cursor-pointer hover:bg-indigo-50/50 group/item' : ''}
            `}
            onClick={() => {
              ...
              {row.isHeader ? '' : (
                <div className="flex flex-col items-end">
                   <div className="flex items-center space-x-1">
                      ...
                      {!row.isTotal && row.code && <ExternalLink className="w-2.5 h-2.5 text-indigo-300 opacity-0 group-hover/item:opacity-100 transition-opacity" />}
                   </div>
                   {!row.isTotal && (
                     <span className={`text-[8px] flex items-center ${idx > 0 && row.values[idx] > row.values[idx-1] ? 'text-emerald-500' : 'text-rose-500'}`}>
                        {idx > 0 && row.values[idx] > row.values[idx-1] ? <TrendingUp className="w-2 h-2 mr-0.5" /> : <TrendingDown className="w-2 h-2 mr-0.5" />}
                        {idx > 0 ? Math.abs(Math.round((row.values[idx] / row.values[idx-1] - 1) * 100)) : 0}%
                     </span>
                   )}
                </div>
              )}
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 pb-6 border-b border-slate-100 dark:border-slate-800">
        <div>
          <div className="flex items-center space-x-3 mb-2">
            <span className="px-2 py-0.5 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 rounded-md text-[10px] font-black uppercase tracking-widest italic tracking-tighter">Financial Reporting</span>
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 shadow-lg shadow-emerald-500/50"></span>
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest italic">Live Ledger Sync</span>
          </div>
          <h1 className="text-3xl font-black text-slate-800 dark:text-white uppercase tracking-tight italic tabular-nums">Mənfəət və Zərər Hesabatı (P&L)</h1>
        </div>
        ...
        <div className="flex items-center space-x-3">
          ...
          <button className="flex items-center space-x-2 px-6 py-3 bg-indigo-600 text-white rounded-2xl font-black text-[11px] uppercase tracking-widest hover:scale-105 transition-all shadow-xl shadow-indigo-500/20 active:scale-95 italic">
            <Download className="w-4 h-4" />
            <span>Excel-ə çıxar</span>
          </button>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 items-end">
           <div className="space-y-3">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-2 flex items-center italic">
                <Calendar className="w-3 h-3 mr-2 text-indigo-500" /> Başlanğıc Tarixi
              </label>
              <input 
                type="date" 
                value={startDate}
                onChange={(e) => setStartDate(e.target.value)}
                className="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-2xl py-3 px-5 text-xs font-black italic shadow-inner outline-none focus:ring-2 focus:ring-indigo-500/20 transition-all"
              />
           </div>
           <div className="space-y-3">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-2 flex items-center italic">
                <Calendar className="w-3 h-3 mr-2 text-indigo-500" /> Bitmə Tarixi
              </label>
              <input 
                type="date" 
                value={endDate}
           ...
           <div className="space-y-3">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-2 flex items-center italic">
                <RefreshCw className="w-3 h-3 mr-2 text-indigo-500" /> Dövriyyə Tezliyi
              </label>
              <div className="flex bg-slate-50 dark:bg-slate-800 p-1.5 rounded-2xl shadow-inner border border-slate-100/50">
                 {['MONTHLY', 'QUARTERLY', 'YEARLY', 'SINGLE'].map(type => (
                   <button 
                     ...
                     className={`flex-1 py-2 rounded-xl text-[9px] font-black uppercase tracking-tight transition-all ${periodType === type ? 'bg-white dark:bg-slate-700 text-indigo-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                   >
           ...
           <div className="pb-1">
              <button className="w-full py-3.5 bg-slate-900 dark:bg-indigo-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:opacity-90 transition-all flex items-center justify-center italic">
                 <Filter className="w-3.5 h-3.5 mr-2" /> Hesabatı Yenilə
              </button>
           </div>
        </div>
      </div>
        ...
        <div className="p-8 border-t border-slate-50 dark:border-slate-800 bg-slate-50/30 dark:bg-slate-800/10 flex justify-between items-center">
           <div className="flex items-center space-x-6">
              ...
              <div className="flex items-center space-x-2">
                 ...
                 <span className="text-[10px] font-bold text-slate-400 italic italic">Datalar son 5 dəqiqə ərzində yenilənib</span>
              </div>
           </div>
           <div className="flex items-center space-x-2">
