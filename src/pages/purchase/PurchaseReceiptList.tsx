...
const PurchaseReceiptList = () => {
  ...
  // Mock Data
  const receipts = [
    { id: '1', number: 'GRN-2024-001', vendor: 'Apple Inc.', date: '2024-04-20', warehouse: 'Bakı Mərkəz', status: 'COMPLETED', items: 12 },
    { id: '2', number: 'GRN-2024-002', vendor: 'Samsung Electronics', date: '2024-04-18', warehouse: 'Gəncə Filial', status: 'DRAFT', items: 5 },
    { id: '3', number: 'GRN-2024-003', vendor: 'Dell Technologies', date: '2024-04-15', warehouse: 'Bakı Mərkəz', status: 'APPROVED', items: 8 },
  ];
  ...
  return (
    <div className="flex flex-col space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div>
          <h1 className="text-2xl font-black text-slate-800 dark:text-white uppercase tracking-tight italic">Malların Mədaxil Reyestri</h1>
          <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1 italic">Anbara daxil olan malların qeydiyyatı</p>
        </div>
        <button 
          onClick={() => navigate('/purchase/receipt/create')}
      ...
      <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-slate-50 dark:border-slate-800 flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-50/30">
          <div className="relative flex-1 max-w-md group">
            ...
            <input 
              type="text" 
              placeholder="Mədaxil № və ya Təchizatçı axtar..."
              value={searchQuery}
            ...
            <tbody className="divide-y divide-slate-50 dark:divide-slate-800 italic-none">
              {receipts.map((item) => (
                <tr key={item.id} className="group hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-all cursor-pointer">
                  ...
                  <td className="px-6 py-5 text-center text-[11px] font-black text-slate-700 dark:text-slate-300">{item.items} SKU</td>
